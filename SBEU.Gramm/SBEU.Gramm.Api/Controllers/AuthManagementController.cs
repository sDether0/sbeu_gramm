...
{
    public class AuthManagementController : ControllerExt
    {
        ...
        [SwaggerOperation("Registration request")]
        [SwaggerResponse(200, "Return auth credentials", typeof(AuthResult))]
        [HttpPost]
        [Route("Register")]
        public async Task<IActionResult> Register([FromBody] UserRegistrationRequest user)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                var newUser = new XIdentityUser() { Email = user.Email, UserName = user.Name };
        ...
        [SwaggerOperation("Refresh token request")]
        [SwaggerResponse(200, "Return new JWT", typeof(AuthResult))]
        [HttpPost]
        [Route("RefreshToken")]
        ...
        [SwaggerOperation("Login request")]
        [SwaggerResponse(200, "Return auth credentials", typeof(AuthResult))]
        [HttpPost]
        [Route("Login")]
        public async Task<IActionResult> Login([FromBody] UserLoginRequest user)
        {
            if (ModelState.IsValid)
            {
                ...
                var existingUser = await _userManager.FindByEmailAsync(user.Email);
                existingUser ??= await _userManager.FindByNameAsync(user.Email);
